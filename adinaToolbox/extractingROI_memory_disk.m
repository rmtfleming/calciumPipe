function [roi] = extractingROI_memory_disk(dirBase,fileExt,stepChannel)
%  DESCRIPTION:
%   Main script for extracting neural activity. This scripts creates mat files
%   that contains a sub-block of the image sequence.
%
%  USAGE:
%
%    function [roi] = extractingROI_memory_disk(dirBase,fileExt,stepChannel)
...
numBlocks = curBlock -1;
disp('Creating Blocks')
creatingDataBlocks(paramsOut);
disp('Data blocks are created!')

for i = 1:numBlocks,
    [InferCells(i)] = automaticROIextraction_disk(paramsOut(i),['~/' paramsOut(1).main.files(1).name(1:end-4) '_tmpBlock_' num2str(i,'%04d') '.mat']);
    delete(['~/' paramsOut(1).main.files(1).name(1:end-4) '_tmpBlock_' num2str(i,'%04d') '.mat'])
end
