function [] = displayExploreCalciumImageFour(data,data1,data2,data3,image,param,idFig)
...
function ButtonDownCallback(src,evedentdata)
   ...
   figure(hTime), subplot(4,1,1),
   plot(initF:step:endF, shiftdim(params.data(row,col,initF:step:endF),1),'b')
   subplot(4,1,2),
   %hold on, 
   plot(initF:step:endF, shiftdim(params.data1(row,col,initF:step:endF),1),'r'),
   subplot(4,1,3),
   plot(initF:step:endF, shiftdim(params.data2(row,col,initF:step:endF),1),'g')
   subplot(4,1,4),
   %hold on, 
   plot(initF:step:endF, shiftdim(params.data3(row,col,initF:step:endF),1),'m'), 
%    subplot(3,1,2)
