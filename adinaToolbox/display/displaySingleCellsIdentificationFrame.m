function [idFig,dispImg] = displaySingleCellsIdentificationFrame(data,cNeurons,idFig)
...
%maxImg = max(data(:));
dispImg = data - min(data(:));
dispImg = dispImg./max(dispImg(:));
imagesc(dispImg), axis square, axis off, colormap 'gray';
