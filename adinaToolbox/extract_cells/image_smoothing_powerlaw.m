function [featNeurons,segmentation] = image_smoothing_powerlaw(dict,time,param)
% DESCRIPTION:
%    Extracts single and unique neurons from base function
%
% USAGE:  [featNeurons,segmentation] = image_smoothing_powerlaw(dict,time,param)
...
%    ISBI 2013. Proceedings, (2013), 1058-1061".
    im0Mask = denoiseGaussPow(dict,param.sizeFilter,param.power);
    imMask = im0Mask>0;
...
if sum(im0Mask(:)~=0)
    ...
    if ~param.flag_watershed
        ...
        segmentation = watershedSegmentationWAPL(im0Mask,1);
